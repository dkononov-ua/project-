...
@Component({
  ...
  styleUrls: ['./carousel-user.component.scss'],
  animations: [animations.top3, animations.bot3, animations.bot5,],
})

export class CarouselUserComponent implements OnInit, OnDestroy {
 ...
 // Зупиняє автоматичну прокрутку при наведенні
  @HostListener('mouseenter', ['$event'])
  pauseScroll(event: any) {
    this.isScrolling = false;
    // this.onWheel(event)
  }

  // Відновлює автоматичну прокрутку при виході миші
  ...
  // // Обробляє прокрутку колеса миші
  // @HostListener('wheel', ['$event'])

  // onWheel(event: WheelEvent) {
  //   const carousel = this.carousel.nativeElement as HTMLElement;
  //   // Збільшуємо чутливість прокрутки
  //   const scrollAmount = event.deltaY * 2; // Множник для збільшення чутливості
  //   carousel.scrollBy({ left: scrollAmount, behavior: 'smooth' });
  //   event.preventDefault(); // Зупиняє стандартне скролювання сторінки
  // }
